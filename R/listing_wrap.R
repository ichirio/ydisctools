...
#'   `<= 0` disables wrapping and returns the text unchanged.
...
#'
#' # width = NULL leaves the cell alone.
#' listing_wrap("Ovarian cancer/BRCA1 mutation", NULL)
#' @export
listing_wrap <- function(text, width, sep = "/") {
  ...
  if (length(text) == 1L) return(.listing_wrap(text, width, sep))
  lapply(text, .listing_wrap, width = width, sep = sep)
}
...
# Wrap one cell.  See listing_wrap() above for the break priority.
.listing_wrap <- function(text, width, sep = "/") {
  ...
  parts <- if (!is.null(sep) && nzchar(sep)) {
    strsplit(text, paste0("(?<=", .listing_quote(sep), ")"), perl = TRUE)[[1L]]
  } else {
    text
  }
