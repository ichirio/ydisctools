...
`ydisctools` is a **grab-bag of small, focused CDISC tools** for R —
SDTM/ADaM dataset building, Define-XML / Pinnacle 21 metadata reading,
ISO 8601 date helpers, controlled-terminology lookups, byte-aware text
splitting, RTF table encoding, and a Sankey treatment-flow plotter.

It is a **Proof of Concept** by design: a workshop where many little
CDISC utilities live side by side, rather than a single-purpose package.
Tools that mature here can graduate into standalone packages — this is
exactly how [rtfreporter](https://github.com/ichirio/rtfreporter) began,
as the RTF-reporting code inside ydisctools before it was spun out on its
own. There is no CRAN submission planned, and the API may change at any
time.

The repository also includes a Sankey plotting utility for
treatment-transition visualization:
...
Full documentation is published with
